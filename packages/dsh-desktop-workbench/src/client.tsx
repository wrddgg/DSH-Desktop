/**
 * DSH Desktop Workbench client plugin: a floating right-side panel with
 * Files / Git / Terminal tabs, backed by the desktop preload bridge
 * (fs / git / pty). Built by scripts/build-plugins.mjs into lib/client.js.
 */
import { basename, formatBytes, gitStatusLabel, joinPath, MAX_FILE_PREVIEW_BYTES, parentOf, stripAnsi } from './workbench-core'
...
const PLUGIN_ID = '@wrddgg/dsh-desktop-workbench'
...
window.__ModuleLoader__?.load({
  ...
  factory: (require) => {
    ...
    interface WorkbenchSnapshot { open: boolean; tab: TabId; cwd?: string }

    const store = {
      open: false,
      tab: 'files' as TabId,
      cwd: undefined as string | undefined,
      listeners: new Set<(snapshot: WorkbenchSnapshot) => void>(),
    ...
    function resolveCwd(sessions: any): string | undefined {
    ...
    function apply(ctx: any): void {
      ...
      function FilesTab() {
        ...
        const load = React.useCallback(async (target: string | undefined) => {
          if (target === undefined) {
            setEntries(null)
        ...
        const openEntry = React.useCallback(async (entry: any) => {
        ...
        const goUp = React.useCallback(() => {
          const parent = dir === undefined ? undefined : parentOf(dir)
          void load(parent)
        }, [dir, load])

        return React.createElement('div', { className: 'dshWorkbenchFiles' },
          React.createElement('div', { className: 'dshWorkbenchList' },
            React.createElement('div', { className: 'dshWorkbenchToolbar' },
          ...
          React.createElement('div', { className: 'dshWorkbenchPreview' },
            selected === null
              ? React.createElement('div', { className: 'dshWorkbenchNotice' }, '点击左侧文件查看内容。')
              : React.createElement(React.Fragment, null,
                  ...
                  selected.error !== undefined
                    ? React.createElement('div', { className: 'dshWorkbenchNotice' }, selected.error)
                    : selected.binary === true
                      ? React.createElement('div', { className: 'dshWorkbenchNotice' }, '二进制文件，无法预览。')
      ...
      function TerminalTab() {
        ...
        React.useEffect(() => {
          void api.pty.available().then((value: boolean) => setAvailable(value))
          const offData = api.pty.onData((id: string, data: string) => {
            if (sessionRef.current !== id) return
            setOutput(previous => previous + data)
          })
          const offExit = api.pty.onExit((id: string, code: number) => {
            if (sessionRef.current !== id) return
        ...
        return React.createElement('div', { className: 'dshWorkbenchTerminal' },
          React.createElement('div', { className: 'dshWorkbenchToolbar' },
            sessionId === null
              ? React.createElement('button', { type: 'button', disabled: busy || available !== true, onClick: () => void start() }, '启动终端')
              : React.createElement('button', { type: 'button', onClick: () => void kill() }, '关闭终端'),
          ...
          React.createElement('pre', { className: 'dshWorkbenchTerminal__output', ref: outputRef },
            output === '' ? '（终端输出将显示在这里）' : stripAnsi(output),
          ),
          React.createElement('div', { className: 'dshWorkbenchTerminal__input' },
      ...
      function GitTab() {
        ...
        const act = React.useCallback(async (task: () => Promise<unknown>) => {
          setBusy(true)
          try {
            await task()
          } finally {
        ...
        const showDiff = React.useCallback(async (entry: any) => {
        ...
        return React.createElement('div', { className: 'dshWorkbenchGit' },
          React.createElement('div', { className: 'dshWorkbenchToolbar' },
            React.createElement('span', { className: 'dshWorkbenchCrumb' }, `分支：${status?.branch ?? '—'}`),
            React.createElement('button', { type: 'button', disabled: busy, onClick: () => void refresh(), title: '刷新', 'aria-label': '刷新' },
              IconRefreshOutline16 ? React.createElement(IconRefreshOutline16, { size: 14 }) : '刷新'),
          ),
          status?.ok !== true
            ? React.createElement('div', { className: 'dshWorkbenchNotice' }, status?.message ?? '读取 git 状态…（当前目录不是 git 仓库时请先选择工作区）')
            : entries.length === 0
              ? React.createElement('div', { className: 'dshWorkbenchNotice' }, '工作区干净，没有未提交的改动。')
              : React.createElement('div', { className: 'dshWorkbenchGit__list' },
                  entries.map((entry: any) => React.createElement('div', {
                    ...
                    entry.staged
                      ? React.createElement('button', { type: 'button', disabled: busy, onClick: () => void act(() => api.git.unstage(store.cwd, [entry.path])) }, '取消暂存')
                      : React.createElement('button', { type: 'button', disabled: busy, onClick: () => void act(() => api.git.stage(store.cwd, [entry.path])) }, '暂存'),
                    React.createElement('button', { type: 'button', disabled: busy, onClick: () => void showDiff(entry) }, '差异'),
                  )),
                ),
          diff !== null
            ? React.createElement('div', { className: 'dshWorkbenchGit__diff' },
                React.createElement('div', { className: 'dshWorkbenchToolbar' },
            ...
            : null,
          React.createElement('div', { className: 'dshWorkbenchGit__commit' },
            React.createElement('input', {
              value: message,
      ...
      function WorkbenchPanel() {
        ...
        const tabs: Array<{ id: TabId; label: string; icon: any; glyph: string | null }> = [
        ...
        return React.createElement('aside', { className: 'dshWorkbenchPanel', role: 'dialog', 'aria-label': '工作台' },
          React.createElement('span', { className: 'dshWorkbenchPanel__resize', onPointerDown: resize, 'aria-hidden': 'true' }),
          React.createElement('header', { className: 'dshWorkbenchPanel__header' },
          ...
          React.createElement('div', { className: 'dshWorkbenchPanel__body' },
            snapshot.tab === 'files' ? React.createElement(FilesTab)
              : snapshot.tab === 'git' ? React.createElement(GitTab)
                : React.createElement(TerminalTab),
          ),
          React.createElement('footer', { className: 'dshWorkbenchPanel__footer' },
            React.createElement('span', null, snapshot.cwd ?? '未绑定工作区'),
          ),
        )
      }

      function ComposerTrigger(props: any) {
        ensureStyles()
        React.useEffect(() => {
      ...
      function SidebarTrigger(props: any) {
        ensureStyles()
        React.useEffect(() => {
          const sessions = typeof props?.useSessions === 'function' ? props.useSessions() : undefined
          store.setCwd(resolveCwd(sessions))
